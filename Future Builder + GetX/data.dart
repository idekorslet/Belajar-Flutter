...
class Helper {
  ...
  Future downloadData({required String apiUrl}) async {
    List products = await loadData(apiUrl: apiUrl);
  ...
  void _divideDataByCategory() {
    ...
    for (int i = 0; i < _allData.length; i++) {
      ...
      _productByCategoryIndex.add([currentIndex, [title, category, price]]);
    }
    ...
    // looping sebanyak jumlah kategori
  ...
  List filterDataByCategory(int categoryIndex) {
    List result = [];
    print(_dataByCategory[categoryIndex]);
  ...
  List get allData => _productByCategoryIndex;
}
